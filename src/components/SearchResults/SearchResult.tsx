...
const SearchResult: React.FC = () => {
  ...
  const colors = [...new Set(productDetails.flatMap((product) => product.colorcode))];
  ...
  useEffect(() => {
    const fetchProductDetails = async (ids: string[]) => {
      try {
        ...
        const chunks = [];
        for (let i = 0; i < ids.length; i += maxConcurrentRequests) {
          const chunk = ids.slice(i, i + maxConcurrentRequests);
          chunks.push(chunk);
        }
        const productDetails: Product[] = [];
        let flattenedArray: Product[] = [];
        for (const chunk of chunks) {
          const promises = chunk.map((id) =>
            fetch(`http://localhost:3000/products/${id}`).then((response) => response.json())
          );
          const chunkProductDetails = await Promise.all(promises);
          productDetails.push(...chunkProductDetails);
          flattenedArray = productDetails.flat();
        }
        setProductDetails(flattenedArray);
      } catch (error) {
  ...
  const handlePriceSortasc = () => {
    const sortedProducts = filteredProductsList.slice().sort((a, b) => a.price - b.price);
    setFilteredProductsList(sortedProducts);
    setSortOrder('asc');
  }

  const handlePriceSortdesc = () => {
    const sortedProducts = filteredProductsList.slice().sort((a, b) => b.price - a.price);
    setFilteredProductsList(sortedProducts);
    setSortOrder('desc');
  }
  ...
  const handleFilterSubmit = () => {
    if (productDetails.length > 0) {
      ...
      const filteredProducts = productDetails.filter((product) => {
        ...
        const nameMatch = searchName ? productName.includes(searchName.toLowerCase()) : true;
      ...
      const updatedFilteredProductsList = products.filter((product) => uniqueNames.includes(product.name));
      ...
      <div className={`${styles["container"]}`}>
        <div className={styles["wraper-filter"]}>
          <div className={styles["wrap"]}>
            <a
              className={`link-dark text-decoration-none ${styles["filter-toggle"]}`}
              onClick={handleFilterToggle}
            >
              <b style={{ userSelect: "none" }}>Bộ lọc</b>
            </a>
            <div className={`${styles["filter-container"]} ${showFilter ? styles["filter-container-open"] : ""}`}>
              <form>
                <div className="row my-2 filter">
                    ...
                    <label>
                      ...
                      <div className="color-container">
                        {colors.map((color) => (
                          <label
                            key={color}
                            style={{ backgroundColor: color, position: 'relative', opacity: 0.8 }}
                  ...
                  <div className="col-md-3 col-lg-3 select-size">
                    <label>
                      <span style={{ userSelect: "none" }}>Size:</span>
                      <div className={`${styles["size-container"]}`}>
                        {sizes.map((size) => (
                          <label key={size}
                            style={{ backgroundColor: isSizeSelected(size) ? "antiquewhite" : "transparent", }}
                          >
                            <input
                              type="checkbox"
                              value={size}
                  ...
                  <div>
                    <span style={{ userSelect: "none" }}>Price: <b>{minValue}VNĐ - {maxValue}VNĐ</b></span>
                  </div>
                  <div className={`col-md-5 col-lg-3 ${styles["select-price"]}`}>
                    <input
                      type="range"
                      className={`${styles["price-min"]} ${styles["range-slider"]}`}
                      min={0}
                      max={getMaxPrice()}
                    ...
                    <input
                      type="range"
                      className={`${styles["price-max"]} ${styles["range-slider"]}`}
                      min={0}
                      max={getMaxPrice()}
                      value={maxValue}
                      step="10000"
              ...
              <a href="#" onClick={handleFilterSubmit} className={styles["filter-button"]}>Lọc</a>
              <div className={styles["sort-section"]}>
                <span style={{ userSelect: "none" }}>Sắp xếp theo:</span>
                <a href="#" onClick={handlePriceSortasc} className={`${sortOrder === 'asc' ? styles['active'] : ''}`}>
                  <span style={{ userSelect: "none" }}>tăng dần</span>
                </a>
                <a href="#" onClick={handlePriceSortdesc} className={`${sortOrder === 'desc' ? styles['active'] : ''}`}>
                  <span style={{ userSelect: "none" }}>giảm dần</span>
                </a>
              </div>
            </div>
          </div>
            ...
            <div className={`${styles.content}`}>
              {filteredProductsList.length > 0 ? (
                filteredProductsList.map((product: Product) => (
                  <ProductShow key={product.id} id={product.id} name={product.name} price={product.price} imgUrl={product.imageurl} />
                ))
              ) : (
